...
// to check _out the generated [API documentation][ap] and the [Usage Guide][ug]. We've
...
// to work with Git at this level, check _out [Chapter 9][pg] of the Pro Git book.
//
...
int main(string[] args)
{
    ...
    data  = cast(const(ubyte)*)git_odb_object_data(obj);
    ...
    writefln("object length %s, type %s.", cast(int)git_odb_object_size(obj), str_type);
    ...
    // the `git_otype` enum.
    string blobData = "test data";
    git_odb_write(&oid, odb, blobData.ptr, blobData.length, git_otype.GIT_OBJ_BLOB);

    // Now that we've written the object, we can check _out what SHA1 was generated when the
    // object was written to our database.
    git_oid_fmt(_out.ptr, &oid);
    writefln("Written Object: %s\n", to!string(_out.ptr));
    ...
    writefln("Author: %s (%s)", author.name, author.email);
    ...
    for (p = 0; p < parents; p++)
    {
        ...
        git_oid_fmt(_out.ptr, git_commit_id(parent));
        writefln("Parent: %s", to!string(_out.ptr));
    ...
    git_signature_new(cast(git_signature**)&author, "Scott Chacon", "schacon@gmail.com", 123456789, 60);
    git_signature_new(cast(git_signature**)&cmtter, "Scott A Chacon", "scott@github.com", 987654321, 90);
    ...
    git_oid_fmt(_out.ptr, &commit_id);
    writefln("New Commit: %s\n", to!string(_out.ptr));
    ...
    writefln("Entry name: %s\n", to!string(git_tree_entry_name(entry))); // "hello.c"
    ...
    // find _out its exact size in bytes
    writefln("Blob Size: %s", git_blob_rawsize(blob)); // 8
    ...
    while (git_revwalk_next(&oid, walk) == 0)
    {
        ...
        writefln("%s (%s)", to!string(cmsg), to!string(cauth.email));
    ...
    // and mode which map to the tree objects that are written _out.  It also has filesystem properties
    ...
    // ### References
    //
    // The [reference API][_ref] allows you to list, resolve, create and update references such as
    // branches, tags and remote references (everything in the .git/refs directory).
    //
    // [_ref]: http://libgit2.github.com/libgit2/#HEAD/group/reference
    ...
    // Now that we have the list of reference names, we can lookup each _ref one at a time and
    // resolve them to the SHA, then print both values _out.
    for (i = 0; i < ref_list.count; ++i)
    {
        refname = ref_list.strings[i];
        git_reference_lookup(&_ref, repo, refname);
        ...
        switch (git_reference_type(_ref))
        {
            ...
            case git_ref_t.GIT_REF_SYMBOLIC:
                writefln("%s => %s\n", to!string(refname), to!string(git_reference_target(_ref)));
                break;

            default:
                assert(0, "Unexpected reference type\n");
        }
    }
